...
class BedRoomControl extends StatefulWidget {
  @override
  State<BedRoomControl> createState() => _BedRoomControlState();
}

class _BedRoomControlState extends State<BedRoomControl> {
  int chosing = -1;
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              if (context.read<BedRoomState>().isControlHumid)
                GestureDetector(
                  onTap: () {
            ...
            Container(
              ...
              child: GridView.count(
                ...
                children: [
                  LightControl(
                    order: 0,
                    isActive: context.read<BedRoomState>().getLightState(0),
                    callBack: context.read<BedRoomState>().changeLightState,
                  ),
                  LightControl(
                    order: 1,
                    isActive: context.read<BedRoomState>().getLightState(1),
                    callBack: context.read<BedRoomState>().changeLightState,
                  ),
                  LightControl(
                    order: 2,
                    isActive: context.read<BedRoomState>().getLightState(2),
                    callBack: context.read<BedRoomState>().changeLightState,
                  ),
                  LightControl(
                    order: 3,
                    isActive: context.read<BedRoomState>().getLightState(3),
                    callBack: context.read<BedRoomState>().changeLightState,
                  ),
                ],
              ),
          ...
          if (chosing == 1)
            MySlider(
              value: context.read<BedRoomState>().humid,
              lowerBound: 5,
              upperBound: 50,
              cautionBound: 40,
              callBack: context.read<BedRoomState>().changeHumidity,
            ),
          // humid controller
        ],
      ),
